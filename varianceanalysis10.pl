...
{	if ($file =~ m/^\w/)
	{	print "processing $file\n";
		...
		{   if ($Genes{$id}{'SIZE'} == 3550)
			...
			my $position = 25;
			while ($position < 3500)
			{   my $window = substr($Genes{$id}{'ntseq'}, $position, $windy); #sets window size for runnng average (20)
			    ...
			    else
			    {
				    ...
				    while ($pos > 0)
				    {   $CGcount += 1; #allows for values to be weighed depending on distance from position within window
				    ...
				    while ($posit > 0)
				    {   $GCcount += 1; #allows for values to be weighed depending on distance from position within window
				    ...
				    while ($posi > 0)
				    {   $GGGcount += 1; #allows for values to be weighed depending on distance from position within window;
		...
		#{   if ($Genes{$id}{'SIZE'} == 3550)
		#	{	my $Nposition = 25;
		#		while ($Nposition < 3500)
		#		    {	my $Nvalue = substr($Genes{$id}{'ntseq'}, $Nposition, 20);
		#			my $Ncount = 0;
		#			if ($Nvalue =~ m/^N/i)
		#			{    $Ncount += 1;
		#			}
